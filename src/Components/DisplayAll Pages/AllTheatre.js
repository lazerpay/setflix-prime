...
export default function AllTheatre() {
    ...
    function fetchMovies() {
        for (let i = 0; i < 5; i++) {
            setLoading(true);
            axios.get(`${apiObj.baseURL}/movie/now_playing?api_key=${apiObj.key}&language=en-US&page=${i + 1}`)
                .then(res => {
                    setLoading(false);
    ...
    return (
        <div className={Styles.container}>
            <Navbar />
            <h2 className={Styles.center}>All Theatre</h2>
            {loading &&
